...
Phases **0–5 are a code freeze**. The freeze **lifts at Phase 6 (IMPLEMENT)** and stays lifted
for Phases 6–10, where writing code is the job. Before Phase 6, the deliverable of *every*
phase is a **document**, never a diff.

## What "writing code" means (all of this is forbidden in Phases 0–5)
...
## What you MAY write in Phases 0–5
...
Creating the feature branch is allowed (Phase 6 pre-flight) — it carries no content.
...
| "The builder will need this file anyway." | Then it belongs in the plan, and the Phase 6 implementer writes it, and a reviewer reviews it. |
...
| "My prompt said to review the code, so I fixed it." | A review returns findings. If a prompt seems to ask a pre-Phase-6 agent for a diff, that prompt is wrong; return the finding instead. |

## If you believe you must write code before Phase 6
...
to the human). A change discovered during Phases 1–5 is *information*, and the pipeline is
built to consume it: it becomes a plan amendment, then a task an implementer picks up in
Phase 6, then a reviewer's diff. Every one of those steps is skipped by an early edit.

## Enforcement (orchestrator)

- **Paste the block below, verbatim, into every subagent prompt dispatched in Phases 0–5** —
  explorers, hardening agents, deep-review finders/verifiers. Any of them can hold an edit
...
- **Check the tree before Phase 6 launches.** `git status --porcelain` must show nothing but
  the plan file. Report the check in the Phase 6 receipt.
- **If an agent broke the freeze:** revert its edits (`git restore`/`git checkout --` the
  ...
  plan amendment. Never carry the edit into Phase 6's base commit — an unreviewed diff that
  predates implementation is exactly what this rule exists to prevent.

## The canonical prompt block (copy verbatim into every Phase 0–5 subagent)
