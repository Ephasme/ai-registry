...
Phase 7's builders commit — so the **feature branch was created before the swarm ran** (never build
on the default branch), and each wave gate committed its tasks as it integrated them. There is
nothing to create here. Confirm the state before pushing:
...
- The commits are there, one per task, in wave order (`git log --oneline`).

## Tidy the history — for the reviewer, not for the record

The swarm's commit sequence is one-per-task in dependency order. That usually reads fine. But the
history exists to serve **the reviewer**, not to document how the work was produced — nobody needs
to be able to reconstruct the wave schedule from `git log`.

So: if the task-by-task sequence tells a clear story, leave it alone. If it doesn't — a task whose
fix rounds left three commits on one file, or a split that made sense to the scheduler and no sense
to a human — **squash it into the commits a human would have made.** Match the repo's existing style
(check `git log`: prefix conventions like `feat:`/`area:`, imperative mood) and honour any trailers
or sign-off it requires. Don't invent attribution the repo doesn't use.
...
## Verification
<the actual build/test/lint commands run, and their result — paste the evidence from Phase 8>

## Notes for the reviewer
<deferred decisions, anything intentionally out of scope, areas wanting a closer look>
```

Write the PR body **for the reviewer, not about the process** — they don't need to know it was built
by a swarm over three waves, and a PR that explains its own machinery reads like it's making excuses.
What they need is what changed, why, and how you know it works. (The one genuine exception: if a
contract was agreed across a wave boundary and is the seam most likely to be wrong, that belongs in
"notes for the reviewer" — not because a swarm built it, but because it's the risky part.)

Capture the **PR URL** — Phase 12 needs it.

**Exit:** an open PR, its URL captured for the handoff.

**Exit receipt example:**
`✅ Phase 9 (PUSH→PR) — confirmed by human — pushed abc-123-rate-limiting (9 commits), opened https://github.com/org/repo/pull/456 ([ABC-123] …, Fixes ABC-123)`
